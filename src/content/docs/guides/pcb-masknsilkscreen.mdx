...
This process is a slight modification of the process for structuring the [LPKF ProMask](https://www.lpkfusa.com/datasheets/prototyping/promask.pdf) soldermask. The original process can be found in the LPFK ProtoLaser U4's How-To document (section 4.3, p. 155), which is located [here](https://gtvault.sharepoint.com/:b:/s/HiveMakerspace/EbRHI5KMoPZFq2Wot9Tkz1UBi6x02PeX6IDQ7XzBW3dKMw?e=7ZzNGW) and on the ProtoLaser itself, in a folder called “ProtoLaser Docs” on the Desktop.
...
3. Carefully lay a single strip of Kapton tape over your design. It should be as tight as possible, with **no** (zero) bubbles underneath, and as straight as possible. It can go beyond the edge of your board as well if needed, though that will just be waste, so try to limit that.
    - I like to use a roller, like a paint roller, or a scraper, to make sure all the bubbles are gone and it's as flat as possible on the substrate.
...
11. Under “Material type”:
    - This process has had success with “Polyimide_0.125mm” material previously, but the resolution is not awesome.
    - You might also try the “ProMask” material type and see if it works better.
    - If you are really keen on using this, I would recommend creating your own material and developing that process. See our [ProtoLaser: New Material](https://gtvault.sharepoint.com/:w:/s/HiveMakerspace/EShrwNPAMH1Cj3cE3pg5h8QB6r3JP-XtLghmQcU96-fNfA?e=Ska4Ud) document for how to do this, and LPKF's [Protolaser - Optimization.pdf](https://gtvault.sharepoint.com/:b:/s/HiveMakerspace/EZGQyfhrC0BItpbSxd_rjRQBzDyNWUubu-RCuJr6a3OI9A?e=nbVu4k) document for optimization.
...
14. Processing is now nearly-identical to the standard process:
    - Compute the toolpath using the standard method (“Toolpaths” > “Compute All”). Press “Close” when it's completed if there are no errors.
    - Lift the ProtoLaser door and place the substrate down onto the bed in the bottom-side up orientation (i.e. the same orientation as you would normally when starting the etching process). Tape it down as normal.
    - Turn on the pilot laser (the black laser button), bring up the Placement window (right click in the design), and align the design's fiducials with those on the substrate as you would normally do.
    - Press the green play button to begin processing. Follow the on-screen instructions as normal. Re-align after flipping.
